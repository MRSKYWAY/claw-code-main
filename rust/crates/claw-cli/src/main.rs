...
use api::{
    resolve_startup_auth_source, ApiError, AuthSource, ClawApiClient, ContentBlockDelta,
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::Models { check } => print_model_catalog(check)?,
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--prompt-file" => {
                let value = args
                    .get(index + 1)
                    .ok_or_else(|| "missing value for --prompt-file".to_string())?;
    ...
    match rest[0].as_str() {
        ...
        "models" => match rest.get(1).map(String::as_str) {
            None => Ok(CliAction::Models { check: false }),
            Some("--check") => Ok(CliAction::Models { check: true }),
            Some(other) => Err(format!("unknown models option: {other} (expected --check)")),
        },
...
fn current_tool_registry() -> Result<GlobalToolRegistry, String> {
    ...
    let plugin_tools = match plugin_manager.aggregated_tools() {
        Ok(tools) => tools,
        Err(error) if should_fallback_to_builtin_tool_registry(&error) => Vec::new(),
        Err(error) => return Err(error.to_string()),
    };
    GlobalToolRegistry::with_plugin_tools(plugin_tools)
}

fn should_fallback_to_builtin_tool_registry(error: &PluginError) -> bool {
    matches!(error, PluginError::Io(io_error) if io_error.raw_os_error() == Some(30))
}
...
fn format_model_report(model: &str, message_count: usize, turns: u32) -> String {
    let aliases = api::MODEL_CATALOG
        .iter()
        .filter(|entry| !entry.alias.starts_with("claude-"))
        .map(|entry| format!("  {:<16} {}", entry.alias, entry.model))
        .collect::<Vec<_>>()
        .join("\n");
    format!(
        "Model
...
fn print_model_catalog(check: bool) -> Result<(), Box<dyn std::error::Error>> {
    let entries = api::MODEL_CATALOG
        .iter()
        .filter(|entry| !entry.alias.starts_with("claude-"))
        .collect::<Vec<_>>();
    if !check {
        for entry in entries {
            println!(
                "{:<16} {:<44} {:?}",
                entry.alias, entry.model, entry.metadata.provider
            );
        }
        println!("\nRun `claw models --check` to make a minimal live availability probe.");
        return Ok(());
    }

    let runtime = tokio::runtime::Runtime::new()?;
    for entry in entries {
        let status = runtime.block_on(check_model(entry.alias, entry.model));
        println!("{:<16} {:<44} {status}", entry.alias, entry.model);
    }
    Ok(())
}

async fn check_model(alias: &str, model: &str) -> String {
    let client = match ProviderClient::from_model(alias) {
        Ok(client) => client,
        Err(error) => return classify_model_check_error(&error),
    };
    ...
    match client.send_message(&request).await {
        ...
        Err(error) => classify_model_check_error(&error),
    }
}

fn classify_model_check_error(error: &ApiError) -> String {
    match error {
        ApiError::MissingCredentials { .. } => "missing credentials".to_string(),
        ApiError::Auth(_) | ApiError::ExpiredOAuthToken => "unauthorized".to_string(),
        ApiError::Api { status, .. } if status.as_u16() == 401 || status.as_u16() == 403 => {
            "unauthorized".to_string()
        }
        ApiError::Api { status, .. } if status.as_u16() == 404 => "unavailable".to_string(),
        ApiError::Api { status, .. } if status.as_u16() == 410 => "retired".to_string(),
        ApiError::Api { status, .. } if status.as_u16() == 429 => "rate limited".to_string(),
        ApiError::RetriesExhausted { last_error, .. } => classify_model_check_error(last_error),
...
) -> Result<ConversationRuntime<DefaultRuntimeClient, CliToolExecutor>, Box<dyn std::error::Error>>
{
    ...
    .with_max_iterations(max_tool_iterations()))
}

fn max_tool_iterations() -> usize {
    env::var("CLAW_MAX_TOOL_ITERATIONS")
        .ok()
...
impl DefaultRuntimeClient {
    fn new(
        ...
        Ok(Self {
            ...
            client: build_runtime_provider_client(&model)?,
            model,
            enable_tools,
            emit_output,
            allowed_tools,
...
fn build_runtime_provider_client(
    ...
    Ok(ProviderClient::from_model_with_default_auth(
        &resolved_model,
        default_auth,
    )?)
}
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    writeln!(
        out,
        "  claw models [--check]                 List aliases or probe availability"
    )?;
    ...
    writeln!(
        out,
        "  claw --model nvidia-agent \"review this Rust crate\""
    )?;
    writeln!(
        out,
...
mod tests {
    use super::{
        classify_model_check_error, describe_tool_progress, filter_tool_specs,
    ...
    #[test]
    fn parses_model_catalog_check() {
        assert_eq!(
            parse_args(&["models".to_string(), "--check".to_string()])
                .expect("models check should parse"),
            CliAction::Models { check: true }
        );
    }

    #[test]
    fn reads_prompt_from_file_without_putting_it_on_the_command_line() {
        let path =
            std::env::temp_dir().join(format!("claw-prompt-file-test-{}.txt", std::process::id()));
        std::fs::write(&path, "remember this entire conversation").expect("write prompt file");
        ...
        .expect("prompt file should parse");
        let _ = std::fs::remove_file(path);
        assert_eq!(
            action,
            CliAction::Prompt {
                prompt: "remember this entire conversation".to_string(),
    ...
    #[test]
    fn model_check_classifies_missing_credentials_without_leaking_values() {
        assert_eq!(
            classify_model_check_error(&ApiError::missing_credentials(
                "Gemini",
                &["GEMINI_API_KEY"]
            )),
            "missing credentials"
    ...
    #[test]
    fn resolves_gemini_aliases_in_args() {
        let args = vec![
            "--model".to_string(),
            "gemini-flash".to_string(),
            "explain".to_string(),
    ...
    fn model_report_uses_sectioned_layout() {
        ...
        assert!(report.contains("gemini-flash     gemini-3.7-flash"));
        assert!(report.contains("nvidia-agent     z-ai/glm-5.2"));
