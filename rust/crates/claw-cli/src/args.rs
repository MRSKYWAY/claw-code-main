...
pub struct Cli {
    #[arg(
        ...
        help = "Model name or alias (for example: sonnet, gemini-flash, nvidia-fast, nvidia-agent)"
    )]
    pub model: String,
...
mod tests {
    ...
    #[test]
    fn accepts_gemini_aliases_in_model_flag() {
        let cli = Cli::parse_from(["claw-cli", "--model", "gemini-flash"]);
        assert_eq!(cli.model, "gemini-flash");

        let cli = Cli::parse_from(["claw-cli", "--model", "gemini-pro"]);
