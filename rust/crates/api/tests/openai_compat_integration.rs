...
#[tokio::test]
async fn provider_client_dispatches_nvidia_requests_from_env() {
    let _lock = env_lock();
    let _api_key = ScopedEnvVar::set("NVIDIA_API_KEY", "nvidia-test-key");
    ...
    let server = spawn_server(
        ...
        vec![http_response(
            "200 OK",
            "application/json",
            "{\"id\":\"chatcmpl_nvidia\",\"model\":\"deepseek-ai/deepseek-v4-flash-0731\",\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"Through NVIDIA\",\"tool_calls\":[]},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":9,\"completion_tokens\":4}}",
        )],
    )
    .await;
    let _base_url = ScopedEnvVar::set("NVIDIA_BASE_URL", server.base_url());

    let client = ProviderClient::from_model("nvidia-fast")
        .expect("NVIDIA provider client should be constructed");
    assert!(matches!(client, ProviderClient::Nvidia(_)));
