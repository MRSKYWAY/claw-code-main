...
pub struct OpenAiCompatConfig {
    ...
    pub sanitize_tool_schemas: bool,
}
...
impl OpenAiCompatConfig {
    ...
    pub const fn nvidia() -> Self {
        Self {
            ...
            // NVIDIA's hosted NIM parser rejects boolean `additionalProperties`
            // values even though they are valid JSON Schema.
...
impl OpenAiCompatClient {
    ...
    pub fn new(api_key: impl Into<String>, config: OpenAiCompatConfig) -> Self {
        Self {
            ...
            sanitize_tool_schemas: config.sanitize_tool_schemas,
        }
    }
        ...
        if !self.message_started {
            ...
            events.push(StreamEvent::MessageStart(MessageStartEvent {
                message: MessageResponse {
                    id: (!chunk.id.is_empty())
                        .then_some(chunk.id.clone())
                        .unwrap_or_else(|| "openai_compat_response".to_string()),
...
fn build_chat_completion_request(request: &MessageRequest, sanitize_tool_schemas: bool) -> Value {
    ...
    if let Some(tools) = &request.tools {
        payload["tools"] = Value::Array(
            tools
                .iter()
                .map(|tool| openai_tool_definition(tool, sanitize_tool_schemas))
                .collect::<Vec<_>>(),
        );
    }
...
fn openai_tool_definition(tool: &ToolDefinition, sanitize_tool_schema: bool) -> Value {
    let mut parameters = tool.input_schema.clone();
    if sanitize_tool_schema {
        remove_boolean_additional_properties(&mut parameters);
    }
    json!({
        "type": "function",
        "function": {
...
fn remove_boolean_additional_properties(value: &mut Value) {
    match value {
        Value::Object(object) => {
            object.retain(|key, value| {
                let is_boolean_additional_properties =
                    key != "additionalProperties" || !value.is_boolean();
                if is_boolean_additional_properties {
                    remove_boolean_additional_properties(value);
                }
                is_boolean_additional_properties
            });
        }
        Value::Array(values) => {
            for value in values {
...
fn normalize_response(
    ...
    for tool_call in choice.message.tool_calls {
        content.push(OutputContentBlock::ToolUse {
            id: (!tool_call.id.is_empty())
                .then_some(tool_call.id)
                .unwrap_or_else(|| format!("tool_call_{}", content.len() + 1)),
...
mod tests {
    ...
    #[test]
    fn streaming_chunks_without_an_upstream_id_receive_a_fallback_id() {
        let chunk = parse_sse_frame(
            "data: {\"choices\":[{\"delta\":{\"content\":\"hello\"},\"finish_reason\":null}]}\n",
        )
        .expect("chunk should parse")
        .expect("frame should contain a chunk");
        let events = StreamState::new("nvidia-test".to_string())
            .ingest_chunk(chunk)
            .expect("chunk should normalize");

        let start = events.first().expect("message start should be emitted");
        let crate::types::StreamEvent::MessageStart(start) = start else {
            panic!("first event should be a message start");
        };
        assert_eq!(start.message.id, "openai_compat_response");
    }
    ...
    #[test]
    fn nvidia_schema_sanitization_removes_boolean_additional_properties() {
        let payload = build_chat_completion_request(
            &MessageRequest {
        ...
        assert!(payload["tools"][0]["function"]["parameters"]
            ...
            .is_none());
        assert!(
            payload["tools"][0]["function"]["parameters"]["properties"]["filter"]
                .get("additionalProperties")
                .is_none()
        );
    }
