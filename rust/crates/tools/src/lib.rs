...
fn tool_is_available_on_current_platform(name: &str) -> bool {
    // The `bash` tool runs POSIX syntax and caused repeated failures in native Windows Claw.
    // PowerShell remains available there, while Unix hosts retain their normal shell tool.
    !(cfg!(windows) && name == "bash")
}
...
fn permission_mode_from_plugin(value: &str) -> PermissionMode {
    match value {
        ...
        _other => PermissionMode::ReadOnly,
    }
}
...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "Agent",
            description: "Launch an asynchronous specialized agent task and persist its handoff metadata. Roles include Explorer, Architect, Coder, Frontend, Backend, Tester, Reviewer, and Debugger.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "description": { "type": "string" },
...
fn resolve_agent_model(model: Option<&str>, subagent_type: &str) -> String {
    ...
    if std::env::var_os("NVIDIA_API_KEY").is_some_and(|key| !key.is_empty()) {
        return match subagent_type {
            "Explore" | "claw-guide" => "nvidia-fast",
            "Plan" | "Architect" => "nvidia-plan",
            "Reviewer" if has_gemini_api_key() => "gemini-flash",
            "Reviewer" => "nvidia-fast",
            "Verification" | "Tester" | "Debugger" | "Coder" | "Frontend" | "Backend"
            | "statusline-setup" => "nvidia-agent",
            _ => "nvidia-agent",
        }
        .to_string();
    }
    ...
    if has_gemini_api_key() {
        return match subagent_type {
            "Explore" | "claw-guide" | "Reviewer" => "gemini-flash",
            _ => "gemini-pro",
        }
        .to_string();
    }
...
fn has_gemini_api_key() -> bool {
    std::env::var_os("GEMINI_API_KEY").is_some_and(|key| !key.is_empty())
}

fn max_iterations_for_subagent(subagent_type: &str) -> usize {
    match subagent_type {
        "Explore" => 12,
        "Plan" | "Architect" | "Reviewer" => 10,
        "Verification" | "Tester" | "Debugger" => 14,
        "claw-guide" | "statusline-setup" => 8,
        "Coder" | "Frontend" | "Backend" => 16,
        _ => 16,
    }
}
...
fn allowed_tools_for_subagent(subagent_type: &str) -> BTreeSet<String> {
    ...
    let mut tools = match subagent_type {
        "Explore" => vec![
            "read_file",
            "glob_search",
            "grep_search",
        ...
        "Verification" | "Tester" | "Debugger" => vec![
            "bash",
            "read_file",
            "glob_search",
            "grep_search",
        ...
        "Coder" | "Frontend" | "Backend" | "statusline-setup" => vec![
            "read_file",
            "write_file",
            "edit_file",
            "glob_search",
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "architect" | "architecture" => String::from("Architect"),
        "coder" | "coding" | "implementer" => String::from("Coder"),
        "frontend" | "frontender" => String::from("Frontend"),
        "backend" | "backender" => String::from("Backend"),
        "tester" | "test" | "testing" => String::from("Tester"),
        "reviewer" | "review" | "codereviewer" => String::from("Reviewer"),
...
fn detect_powershell_shell() -> std::io::Result<PathBuf> {
    #[cfg(windows)]
    {
        let system_shell = std::env::var_os("SystemRoot")
            .or_else(|| std::env::var_os("WINDIR"))
            .map(PathBuf::from)
            .map(|root| root.join("System32/WindowsPowerShell/v1.0/powershell.exe"));
        if let Some(path) = system_shell.filter(|path| path.is_file()) {
            return Ok(path);
        }
        if command_exists("pwsh.exe") {
            return Ok(PathBuf::from("pwsh.exe"));
        }
    ...
    Err(std::io::Error::new(
        ...
        "PowerShell executable not found (checked the Windows system path and PATH)",
    ))
}
...
mod tests {
    ...
    use super::{
        ...
        execute_tool, final_assistant_text, max_iterations_for_subagent, mvp_tool_specs,
        persist_agent_terminal_state, push_output_block, resolve_agent_model, AgentInput, AgentJob,
    ...
    fn skill_loads_local_skill_prompt() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let skill_home = temp_path("skill-home");
        let skill_dir = skill_home.join("skills").join("help");
        ...
        std::env::set_var("CODEX_HOME", &skill_home);
        let result = execute_tool(
            "Skill",
            &json!({
                "skill": "help",
        ...
        let _ = std::fs::remove_dir_all(skill_home);
    }
    ...
    fn agent_tool_subset_mapping_is_expected() {
        ...
        let reviewer = allowed_tools_for_subagent("Reviewer");
        assert!(reviewer.contains("read_file"));
        assert!(!reviewer.contains("bash"));
        assert!(!reviewer.contains("write_file"));

        let coder = allowed_tools_for_subagent("Coder");
        assert!(coder.contains("bash"));
        assert!(coder.contains("write_file"));
        assert!(!coder.contains("Agent"));
    }

    #[test]
    fn agent_routing_uses_nvidia_specialists_when_available() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let original = std::env::var_os("NVIDIA_API_KEY");
        let original_gemini = std::env::var_os("GEMINI_API_KEY");
        std::env::set_var("NVIDIA_API_KEY", "nvidia-test-key");
        std::env::remove_var("GEMINI_API_KEY");

        assert_eq!(resolve_agent_model(None, "Explore"), "nvidia-fast");
        assert_eq!(resolve_agent_model(None, "Plan"), "nvidia-plan");
        assert_eq!(resolve_agent_model(None, "Architect"), "nvidia-plan");
        assert_eq!(resolve_agent_model(None, "Coder"), "nvidia-agent");
        assert_eq!(resolve_agent_model(None, "Tester"), "nvidia-agent");
        assert_eq!(resolve_agent_model(None, "Verification"), "nvidia-agent");
        assert_eq!(resolve_agent_model(None, "general-purpose"), "nvidia-agent");
        assert_eq!(
            resolve_agent_model(Some("gemini-pro"), "Explore"),
            "gemini-pro"
        );
        assert_eq!(max_iterations_for_subagent("Explore"), 12);
        assert_eq!(max_iterations_for_subagent("general-purpose"), 16);

        std::env::remove_var("NVIDIA_API_KEY");
        std::env::set_var("GEMINI_API_KEY", "gemini-test-key");
        assert_eq!(resolve_agent_model(None, "Explore"), "gemini-flash");
        assert_eq!(resolve_agent_model(None, "Architect"), "gemini-pro");
        assert_eq!(resolve_agent_model(None, "Coder"), "gemini-pro");

        match original {
            Some(value) => std::env::set_var("NVIDIA_API_KEY", value),
