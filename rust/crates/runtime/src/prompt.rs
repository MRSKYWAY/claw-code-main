...
fn get_simple_system_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "Use the Agent tool for independent subproblems. It launches asynchronously and returns manifest and output paths under .claw-agents.".to_string(),
        "For multi-agent work, parallelize only read-only discovery or review agents (Explorer, Architect, Reviewer). Run Coder, Frontend, and Backend agents one at a time unless they have isolated worktrees.".to_string(),
        "Give each subagent a narrow task and relevant paths; read its persisted result before synthesizing or making edits.".to_string(),
