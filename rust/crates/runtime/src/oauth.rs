...
fn credentials_home_dir() -> io::Result<PathBuf> {
    ...
    match (std::env::var_os("HOMEDRIVE"), std::env::var_os("HOMEPATH")) {
        (Some(drive), Some(path)) => Ok(PathBuf::from(drive).join(path).join(".claw")),
        _ => Err(io::Error::new(
            io::ErrorKind::NotFound,
            "no home directory is configured; set CLAW_CONFIG_HOME, HOME, or USERPROFILE",
        )),
    }
}
...
mod tests {
    ...
    #[test]
    fn credentials_path_uses_windows_userprofile_when_home_is_absent() {
        let _guard = env_lock();
        ...
        std::env::set_var("USERPROFILE", "C:\\Users\\claw-test");

        assert_eq!(
            credentials_path().expect("credentials path"),
            PathBuf::from("C:\\Users\\claw-test").join(".claw/credentials.json")
        );
